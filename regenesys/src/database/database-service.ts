...
const namespace = cls.createNamespace('sequelize-db');
...
class DatabaseService {
    ...
    private static async getInstance() {
        ...
        const transaction = (task: any) => (namespace.get('transaction') ? task() : sequelize.transaction(task));
        ...
        fs.readdirSync(config.dbModelsDir)
            ...
            .forEach((file: string) => {
                const model = require(path.join(config.dbModelsDir, file))(sequelize, Sequelize.DataTypes);
                db[model.name] = model;
            });
